...
## Local-native embedder (K1)

| Path | What it shows |
|---|---|
| [`mooshik-k1-metal/`](mooshik-k1-metal/) | The K1 spike's three numbers on Apple silicon (M3 Pro), measuring in-process BGE-M3 through candle 0.11 against the rig's own llama.cpp q8_0 as an independent reference. **Parity passes outright** — median cosine 0.99972 over 82 texts spanning ten languages and 32 B → 8 KiB, with every individual item above the 0.99 gate, so no fastembed retreat is recorded. **Throughput's falsifier does not fire**: candle Metal clears half the 110–141 items/s baseline at both probe sizes and beats the reference outright when embeds are batched (190.5 vs 146.2 items/s), while candle CPU lands at ~21–27% of baseline and is a fallback rather than a peer. **Cold start is 1.2–1.6 s**, ~15–25× under the ~30 s client-abandon gate, so lazy-load stays optional. Two corrections live here too: `BAAI/bge-m3` ships no safetensors at all, and the batching-not-threading result is what K2's adapter design turns on. Runbook in [`mooshik-k1-metal/README.md`](mooshik-k1-metal/README.md). |

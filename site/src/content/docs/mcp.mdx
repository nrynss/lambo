...
forward every tool call, so each client gets full read, full write, and real read-your-writes
against one graph rather than one client getting memory and the rest getting none. There is
nothing to configure — point each client at `lambo serve --session <id>` the ordinary way.

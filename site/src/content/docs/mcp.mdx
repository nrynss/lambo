...
- Distinct ids get distinct locks and contend honestly: one agent reserving a node another holds gets a conflict.
...
id share locks. Never send a timestamp: the server stamps them. Ordering is yours to manage: a read sees a write only

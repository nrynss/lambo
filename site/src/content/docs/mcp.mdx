...
When the server was started with `serve --ledger`, the payload also carries `ledger_path`, `ledger_written_lines`, and `ledger_dropped_lines`. The dropped count is the one to read first: the call ledger drops lines rather than delay a tool call, so a non-zero value means any count taken from the ledger file is a lower bound. Without `--ledger` these three keys are absent and the payload is unchanged.

...
set, otherwise `$TMPDIR/lambo-<uid>`, otherwise `/tmp/lambo-<uid>`. The directory must be mode
700 and owned by you — `lambo serve` creates it that way and refuses to bind into one that is
not, since a socket in a directory other accounts can reach would let any local account issue
writes against your session. If you see that refusal, remove the directory or set
`XDG_RUNTIME_DIR` to one you own. A very long base directory can also leave no room for a
socket address (the limit is 104 bytes for the whole path); a serve on such a machine still
serves its own client normally and logs the reason, and only the forwarding stops working.

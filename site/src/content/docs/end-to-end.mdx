...
Because each agent call is one deterministic line, a small local model can drive it without a tool schema taking up its context. Over MCP the schemas are small enough that a 0.6B model composed 173 well-formed calls in a three-agent run; models that cannot emit tool calls at all are the real limit, not size. See [Evidence](/lambo/evidence/). Canonization collapses the duplicate observations that many agents produce into single canonical facts.

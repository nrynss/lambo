...
The walkthrough above uses SQLite, so it exercises all three. SQLite scans its stored vectors exactly rather than through an index, which is the right trade while a session's concept count stays small and returns exact nearest neighbours rather than approximate ones. The in-memory store has no vector leg: recall there matches on the words you use rather than on meaning, so a query sharing no vocabulary with a stored concept can miss it.

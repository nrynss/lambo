...
The walkthrough above uses SQLite, so it exercises all three. SQLite scans its stored vectors exactly rather than through an index, which is the right trade while a session's concept count stays small and returns exact nearest neighbours rather than approximate ones. The in-memory store has no vector leg at all: recall there cannot match on meaning under any embedder.

**The walkthrough exercises the vector leg mechanically, not semantically.** Its config is `[embedder] kind = "fixture"`, and the fixture embedder is *deterministic, not semantic*: it hashes the normalized phrase to a pseudo-random unit vector, so two phrasings of the same idea are near-orthogonal unless they happen to fall in its one hardcoded family. Running the walkthrough proves vectors are written, queried, and ranked; it does not show a semantic hit, and the scores you see for non-keyword matches will be flat. For recall by meaning, point `[embedder]` at a real embedder — `kind = "bge_m3"` against a local llama.cpp server. A captured run of exactly that, where a query sharing no vocabulary with a derived concept recalls it, is in [`evidence/mooshik-f-sqlite-bge/`](https://github.com/nrynss/lambo/tree/main/evidence/mooshik-f-sqlite-bge) (that link resolves once the `lambo-for-mooshik` branch merges; the directory is committed there today).

One asymmetry to know when you write your own queries: `derive` embeds the concept *with its context* — the string `"{content} — {origin}"` — while `recall` embeds your query text bare. So a query is compared against the framed string, not against the concept's content alone. With a real embedder this costs a little similarity and the ranking survives it; with the fixture embedder, which has no notion of similarity, it is the difference between a hit and a miss.
...
A second **command line write** on the same session is refused while a `serve` holds the lease. The refusal names the holder, so you know which process to stop.

A second `serve` is different, and is not refused: over stdio it connects to the holder and forwards your calls, so the session still has exactly one writer while every client gets full access. See [MCP tools](/lambo/mcp/#more-than-one-client-on-one-machine) for that. Over HTTP a second `serve` is refused like a command line write.

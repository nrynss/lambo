...
The in-crate candle embedder (`kind = "candle"`, built with `--features embed-candle`) runs BGE-M3 locally with no llama-server. Cold start: ~2-3 s warm load on every start; the very first use additionally fetches ~1.1 GB of weights from hf-hub (~60 s once, then cached) — plan a stdio client's startup timeout accordingly.

...
| Key | Type | Default | Notes |
|---|---|---|---|
| `kind` | string | `bge_m3` | `bge_m3`, `fixture`, or `bedrock`. `bedrock` needs `--features embed-bedrock`. Titan V2 accepts only dim `256`, `512`, or `1024`. Invoke is still blocked until the account is AUTHORIZED. |
...
| `embed-bedrock` | Amazon Titan Text Embeddings V2. The adapter constructs; InvokeModel waits on account authorization (issue #3). Not in `ship`. |

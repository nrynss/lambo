...
These are library settings you pass in Rust when you build a `Memory`. Three of them can also come from the process file. `promotion_policy` is a top-level `lambo.toml` key with a `LAMBO_PROMOTION_POLICY` override, so a `lambo serve` process can select a policy without a code change; `[daemon]` carries the two cadence settings, `canonization_eval_interval` (spelled `canonization_eval_interval_secs` there) and `gc_interval`. Everything else is code-only, and the file refuses it along with any key it does not know. No canonization *threshold* is settable from the file under either policy. See [Configuration](/lambo/config/).
...
| `match_strategy` | `Hybrid` | `Canonical` or `Hybrid` — selects the recall matching scope, whether a derive embeds, **and** the call-time validation rule set (see `MatchStrategy` in the source authority). J3-R2R-7: `Canonical` adds `reject_repeated_observation` and the single-`Hierarchical`-parent rule. |
...
| `canonization_eval_interval` | 60 seconds | How often promotion is evaluated. Also settable from the file, as `[daemon] canonization_eval_interval_secs`. |
...
| `promotion_policy` | `Swarm` | Which promotion policy Stage 1 runs. `Swarm` is spec §3.2 multi-agent convergence — a peer-count session gate and a P90 cut on the peer score distribution. `Solo` (single-writer promotion, C2) scores each concept as `(Sessions × 1.0) + (Human Confirmed × 4.0) + (Valid Actions × 2.0) − (Reverts × 3.0)`, multiplied by its concept type's eviction resistance (Constraint 1.5 … Observation 0.7); sessions are distinct ≥24-hour-apart supports measured on **event time**, and human confirmations enter only through the confirm verb (`Memory::confirm_human`), never an agent write. Under `Solo` the score's bands drive the ladder itself: ≥ 3.0 admits at Candidate, and the Candidate → Venerable and Venerable → Canonical hops are decided by clearing 6.0 / 10.0 instead of the store-evidence stages swarm uses (the Canonical budget cap and re-promotion cooldown still apply). Both values are implemented; `Swarm` stays the default. Selectable from `lambo.toml` or `LAMBO_PROMOTION_POLICY` (see [Configuration](/lambo/config/)); there is no `serve` flag. |
...
Backends are chosen by Cargo feature and configuration, not loaded as plugins. See [Configuration](/lambo/config/) for the feature flags.
...
See [MCP tools](/lambo/mcp/) and [Configuration](/lambo/config/).

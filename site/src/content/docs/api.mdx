...
| `promotion_policy` | `Swarm` | Which promotion policy Stage 1 runs. `Swarm` is spec §3.2 multi-agent convergence — a peer-count session gate and a P90 cut on the peer score distribution — and is the only implemented value. `Solo` (single-writer promotion) is declared but not built; `Config::validate()` refuses it. |

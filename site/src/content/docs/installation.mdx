...
Real clients drive this, not only unit tests. Claude Code completes the handshake and lists all seven tools over stdio. OMP, the Oh My Pi harness, is an MCP client out of the box. A DeepSeek Flash model running under OMP autonomously called derive, recall, and stats against a live CockroachDB session. Pi needs its MCP adapter installed first, after which it drives all seven tools.

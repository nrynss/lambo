...
Set `kind` to `memory`, `sqlite`, or `cockroach`. The kind you choose must be compiled into your binary. If it is not, Lambo fails at startup with a clear message. See [Installation](/installation) for the matching features.
...
Set `kind` to `fixture`, `bge`, or `bedrock`.
...
<Warning>
The HTTP transport has no authentication, no rate limiting, and no cap on the number of sessions it creates. Keep it on localhost. Do not expose it to a network until authentication is added.
</Warning>

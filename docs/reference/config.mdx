...
| Key | Type | Default | Notes |
|---|---|---|---|
| `kind` | string | `bge_m3` | `bge_m3`, `bedrock`, or `fixture`. |
...
The HTTP transport binds to `127.0.0.1` on port `7700`. Change them with `--bind` and `--port` on `lambo serve`.

<Warning>
The HTTP transport has no authentication, no rate limiting, and no cap on the number of sessions it creates. Keep it on loopback. Do not expose it to a network until authentication is added.
</Warning>

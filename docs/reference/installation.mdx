...
Prebuilt binaries are the primary install path. Each release carries one `lambo`
binary per platform plus a SHA-256 checksum file. Releases are published for
Linux x86_64, Linux arm64, macOS arm64, macOS x86_64, and Windows x86_64, with
the full adapter feature set compiled in.
...
SHA-256 checksum against the published checksum, and installs it to
`~/.local/bin`. To pin a specific version or choose the install directory:
...
Confirm the install with `lambo --version`.

On Windows, download `lambo-<version>-windows-x86_64.exe` and its `.sha256` file
from the release page instead. You can also install from source with cargo:

```bash
cargo install --git https://github.com/nrynss/lambo.git
```

This compiles the master branch, so it may be newer than the latest tag. Use the
prebuilt binary to get exactly the tested, checksummed release.

Each release binary carries every adapter it was compiled with, so you switch
stores by editing `lambo.toml`, not by downloading a different binary. The
runtime caveat is that the adapter code is compiled in, but its backing service
must be reachable (a `llama-server` for BGE, a cluster for Cockroach).

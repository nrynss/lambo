...
Recall combines keyword matching with vector similarity, then expands through the graph. Which of those legs run depends on your store. Every concept your agents write through `lambo_derive` and `lambo_record_action` keeps its embedding, so a store with vector search finds memory your own agents wrote, not just memory you loaded in bulk.

...
| An agent consulting memory | A model asked about deleting a security group, calling `lambo_recall` and `lambo_inspect` unprompted, and refusing. See [Agent skill](/agent-skill) |
...
tool calls are the model's own choice. The [Agent skill](/agent-skill) page has

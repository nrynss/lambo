...
<Note>
The command line is in active development. Run `lambo <command> --help` for the current, authoritative flags on your build.
</Note>
...
For the meaning of each argument, the value limits, and the concept types, see [MCP tools](/mcp). Both surfaces run the same underlying operations. See [Library API](/api) for the type they share.

...
lambo derive: conflict: session demo is already held by another writer (agent-a@host#10492) — it acquired the single-writer lease 66s ago and is still refreshing it. Spec §2.2 is one writer per session; refusing to open a second. If that holder is wedged, an operator can force a takeover: DELETE FROM session_leases WHERE session_id = '<session>';
$ echo $?
1
```
...
`lambo demo` is reserved for a scripted two-agent scenario. `--scenario` defaults to `rest-api`.

<Note>
This command is not built yet. It prints the scenario name and exits `0` without touching memory. Use the walkthrough in [End to end](/end-to-end) instead.
</Note>

...
Writes flow through one in-memory graph. They land in a write-behind log and flush to the durable store on an interval, so durability is eventual, not immediate. Call `lambo_stats` on the writer to see the current lag. Each flush goes out as a few batched statements rather than one statement per change, so a burst of writes costs a handful of round trips to the store instead of one per change.
...
Text is checked on the way in. Lambo refuses control characters other than tab and newline, and refuses invisible formatting characters such as bidi overrides and zero-width spaces, so nothing that a reviewer cannot see reaches the graph. See [What Lambo accepts in text](/cli#what-lambo-accepts-in-text).
...
It serves on port `7710` (loopback, unauthenticated by default) and only reads — live recall, the canonization feed, and durable counts — so it never contends for the writer lease. See [Command line](/cli) for its flags.

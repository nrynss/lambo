...
impl HotList {
    ...
    /// stays within `max`). Callers that don't care should bind or
    /// `let _ =` the result.
...
mod tests {
    ...
    fn bound_enforced_under_mixed_severity_overflow() {
        ...
        let mut list = HotList::with_max(3);
        list.insert(entry(nid(1), Condition::Conflict));
        list.insert(entry(nid(2), Condition::Drift));
        list.insert(entry(nid(3), Condition::StaleSession));
    ...
    fn severity_outranks_recency() {
        ...
        list.insert(entry(nid(1), Condition::StaleSession));
        list.insert(entry(nid(2), Condition::Drift));
        list.insert(entry(nid(3), Condition::Conflict));
    ...
    fn recency_breaks_ties_within_a_condition() {
        let mut list = HotList::new();
        list.insert(entry(nid(1), Condition::Conflict));
        list.insert(entry(nid(2), Condition::Conflict));

...
* **F4 — now measured, not scheduled** (`evidence/mooshik-f4-cockroach/README.md`): close-time
  ...
  **Scope note:** this envelope is an *interactive-path* constraint. Mooshik's bootstrap
  ingest deliberately does not ride this path and is not bounded by K ≤ ~150 — the
  decided direction (bulk verb, off-lease, checkpoint/resume; unbuilt) lives in

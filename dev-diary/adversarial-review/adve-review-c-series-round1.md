...
- **Agent:** `CSeriesRemediationR1` (worker, full access, confined to the
  `c-series` worktree).
- **Remediation commit:** `aa3406f` — `fix(c-series): round-1 remediation`
  (evidence + docs fixes and the committed artifact additions described
  below; no Rust production code touched, `scripts/` docstring-only).
- **Per-finding disposition:**
  - **CS-R1-1 (P1) — FIXED.** The fabricated `Memory session closed (tail
    flushed) mutations=332` quote and the "332-mutation drain in ~1.4 s"
    figure are removed from `evidence/concurrency/README.md` and
    `dev-diary/notes/concurrency-capture.md`. Both now state the artifact
    reality: the shutdown transcript is `shutdown signal received, winding
    down` (line 1711) → `lambo serve: session closed, tail durable` (line
    1712) with no `Memory session closed (tail flushed)` line anywhere —
    which, given `src/memory.rs` logs that exact line whenever `close()`
    flushes a non-empty tail under `RUST_LOG=lambo=info`, proves the
    close-time tail was empty; the verified surplus is 21 interactions (store
    862 vs ledger-ok 841; 11 `ok` responses landed after the SIGTERM
    timestamp); the close drain was a no-op on SQLite; `CLOSE_GRACE` was not
    tested to its limit — stated honestly.
  - **CS-R1-2 (P2) — FIXED.** `concurrency-capture.md` and the runbook now
    carry only artifact numbers, re-verified by recomputation before writing:
    3423 calls / 1311 ok / 841 writes (468 derive + 373 record_action) /
    derive created·matched 555·815 / record_action created·edges 911·5506 /
    store interactions 862 (862−841 = 21) / ledger-created 1466 vs store 1359
    (1466−1359 = 107 == `concepts_collected=107`). The figures 830/1454/3455/
    1303/585/870 (and the "3455 ledger calls" in the wire-hygiene section)
    are gone.
  - **CS-R1-3 (P2) — FIXED (preferred path).** The `gc_interval=1` control
    run's artifacts — which existed uncommitted in `/tmp/c-series-scratch/`
    — are committed under `evidence/concurrency/control-gc/`:
    `gcproof.toml` (`[daemon] gc_interval = 1`), `gcproof.log` (55 GC sweeps,
    `concepts_collected` summing to 11), `gcproof-ledger.jsonl` (198 calls, 2
    workers, full phase set), `gcproof.db` (store), `durability-gcproof.txt`
    (regenerated by the committed `check_durability.py`: 217 ledger-created −
    206 store concepts = 11 == GC sum 11; interactions 76 = 76 MATCH), and
    `run-gcproof.json` (run metadata). Both the runbook and the note cite
    these artifacts for the 11 = 11 claim.
  - **CS-R1-4 (P3) — FIXED.** Runbook now says 80 unique action nodes: 80
    `ok` at-cap `record_action` calls in the burst window with 80 distinct
    action strings, and the store holds exactly 80 `burst action %` concepts
    (both re-verified).
  - **CS-R1-5 (P3) — FIXED.** The unsupported 133 ms figure is replaced with
    the committed transcript gap: `shutdown signal received`
    20:50:24.561330Z → `session closed, tail durable` 20:50:24.562066Z
    (~0.7 ms), from `evidence/swarm/stderr-serve-20260817-205024.log`.
  - **CS-R1-6 (P3) — FIXED.** "Bounded to 120 calls each" is gone; the
    docstring, `run_loop` docstring, runbook and loadtest README now say
    "free-run for the first `--overdrive-calls` calls, then paced at ~20 rps
    per worker" (the loadtest README phase list now includes the overdrive
    phase, which it previously folded into the burst bullet). Ledger-verified
    cross-check retained: 1800 calls in the first 2.1 s of the run.
  - **CS-R1-7 (P3) — FIXED.** "Zero refusals" is replaced with the precise
    truth in both the note and the runbook: 1673 of 1682 rate-limit 429s are
    in the overdrive window, 9 fall in the main window's opening (boundary
    carryover after the overdrive exhausted the limiter's burst budget), and
    21 valid in-window `lambo_derive` calls returned tool-level `store error`
    responses (the 21 `hybrid derive could not commit` ERROR lines at
    `stderr-20260817-204139.log:1689-1709`). The intended claim — rate-limit
    refusals never crowd out the measurement — is restated with those
    numbers.
  - **CS-R1-8 (P3) — FIXED.** Both probes are now committed artifacts under
    `evidence/swarm/probes/`: `omp-harness-garbled-tool-call.txt` (OMP-harness
    probe re-run 2026-08-17T21:05Z against the same `lambolocal` provider
    config and model server: prose fabricating a `lambo_derive` CLI, zero
    `tools/call` requests on the probe server, 0 store interactions) and
    `raw-tools-probe.json` (raw `/v1/chat/completions` with a `tools` array:
    `finish_reason=stop`, no `tool_calls`), plus `probes/README.md` recording
    the setup and that these are re-run transcripts of the original
    uncommitted observations. The card scores 4.13/3.73 — present in no
    committed artifact and not reproducible from the ledger — are dropped
    from the runbook (the quoted card texts remain; they appear verbatim in
    the swarm ledger and the PNGs are pixel-verified renders). The metrics
    table now footnotes the unparseable-turn rate (54 of 218 model turns,
    25%, recorded as `model_reply` with `parsed_concepts: 0`) and defines
    "Model errors 0" (0 `model_error` records; HTTP/transport failures only).
- **Verification performed:** every corrected number was re-derived from the
  committed artifacts before writing (ledger parses, store readbacks, stderr

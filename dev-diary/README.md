...
| P9 | 2 / 2 required (+1 / 1 optional) | **T9.1 docs DONE** (merged `7b0a9f8`, on `main` and pushed; the "do not push" release blocker is discharged: three releases published, install path live) · **T9.2 diagram DONE** (mermaid in [README.md](../README.md) §Architecture + the site topology) · **Video NOT STARTED** (tracked as D2, needs D1 first). T9.3 was rewritten the same day so the requirement is the swarm itself rather than a named model or rig. Not covered, recorded in the task: `reserve` coordination was never exercised, and the headline throughput figures come from the fallback loop, so the model-chosen number is the agentic run's 1120 tasks/hour at dedup 0.857. See [PHASE-9-ship.md](PHASE-9-ship.md) |

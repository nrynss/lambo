...
| P9 | 2 / 3 required (+1 / 1 optional) | **T9.1 docs DONE** (merged `7b0a9f8`, on `main` and pushed; the "do not push" release blocker is discharged: three releases published, install path live) · **T9.2 diagram DONE** (mermaid in [README.md](../README.md) §Architecture + the site topology) · **Video NOT STARTED** (tracked as D2, needs D1 first) · **T9.5 Devpost NOT STARTED** (`dev-diary/notes/devpost.md` does not exist yet; D3 already wrote the submission *text* into README + `site/.../hackathon.mdx`). Optional T9.6 (swarm benchmark) **SATISFIED 2026-08-18 by the C-series C5 work**: a swarm ran against one session with tasks/hour + dedup captured, and the LFM2 no-tool-call leg was diagnosed rather than left hanging. T9.6 was rewritten the same day so the requirement is the swarm itself rather than a named model or rig. Not covered, recorded in the task: `reserve` coordination was never exercised, and the headline throughput figures come from the fallback loop, so the model-chosen number is the agentic run's 1120 tasks/hour at dedup 0.857. See [PHASE-9-ship.md](PHASE-9-ship.md) |
...
| **D1–D3** deployment & submission | [notes/deployment-and-submission.md](notes/deployment-and-submission.md) | **D3 done** (docs + submission text, landed early). **D1 done 2026-08-18**: the exhibit host is a clean product of `launch_exhibit_ec2.py`, old instance terminated, new one launched from zero, Elastic IP re-associated so `lambo.nryn.dev` needed no DNS change, serving 0.2.2 (evidence `evidence/d1-redeploy/`) — "rebuildable from the scripts alone" is now supportable. **D2 recording NOT STARTED**, no longer blocked. The spec §12.4 checklist now lives in this note too |

**Critical path to submission:** D1 → D2 (video) → T9.5, with T9.2 already met. A

...
| Tue Aug 18 | Submit **before** 5:00 pm ET | ✅ **DELIVERED.** C1–C5 concurrency capture closed, D1 clean redeploy, D2 video published ([watch](https://www.youtube.com/watch?v=dfbMvf_YwFo)), Devpost entry up (https://devpost.com/software/lambo), site gained a Canonization page and the hackathon page's last `Not yet` row cleared. `demo/` was gitignored as a video-asset folder: its operator page moved to [`../docs/demo.md`](../docs/demo.md) and `demo/LIVE-RUNBOOK.md` went local-only (paths in [PHASE-8-surface.md](PHASE-8-surface.md) §T8.4 Owns). Open items are recorded as open: Bedrock authorization (external) and multi-writer coordination (scoped out of v0.2) |
...
SQLite, reservations, and drift all shipped. The one open task, **T7.1 Titan, was gated on
an external AWS authorization, not on our schedule** — its adapter PR went up regardless
([#6](https://github.com/nrynss/lambo/pull/6), unmerged) and the access request never cleared. BGE-M3 remains the default dense
path, so no deliverable depended on that gate clearing, and none does now.
...
|---|---|---|
| P0 | 3 / 4 | **GO on Rust** — T0.1–T0.3 done; T0.4 **closed as blocked, not done**: the Bedrock use-case form was never approved (`evidence/bedrock-blocked.txt`). Skeletal adapter parked as unmerged PR [#6](https://github.com/nrynss/lambo/pull/6); not counted done |
...
| P9 | 2 / 2 required (+1 / 1 optional) | **T9.1 docs DONE** (merged `7b0a9f8`, on `main` and pushed; the "do not push" release blocker is discharged: three releases published, install path live) · **T9.2 diagram DONE** (mermaid in [README.md](../README.md) §Architecture + the site topology) · **T9.3 video DONE (2026-08-18)** — [Lambo: Durable canonical memory for agents](https://www.youtube.com/watch?v=dfbMvf_YwFo), published and linked from the README header, the hackathon page and the evidence page (tracked as D2; D1 cleared first). T9.3 was rewritten the same day so the requirement is the swarm itself rather than a named model or rig. Not covered, recorded in the task: `reserve` coordination was never exercised, and the headline throughput figures come from the fallback loop, so the model-chosen number is the agentic run's 1120 tasks/hour at dedup 0.857. See [PHASE-9-ship.md](PHASE-9-ship.md) |
...
| **D1–D3** deployment & submission | [notes/deployment-and-submission.md](notes/deployment-and-submission.md) | **D3 done** (docs + submission text, landed early). **D1 done 2026-08-18**: the exhibit host is a clean product of `launch_exhibit_ec2.py`, old instance terminated, new one launched from zero, Elastic IP re-associated so `lambo.nryn.dev` needed no DNS change, serving 0.2.2 (evidence `evidence/d1-redeploy/`) — "rebuildable from the scripts alone" is now supportable. **D2 done 2026-08-18**: video recorded and published ([https://www.youtube.com/watch?v=dfbMvf_YwFo](https://www.youtube.com/watch?v=dfbMvf_YwFo)), Devpost entry up at https://devpost.com/software/lambo. All three deployment and submission tasks closed |

**Critical path to submission: CLOSED (2026-08-18).** D1 → D2 (video) → Devpost all done, tracked in [notes/deployment-and-submission.md](notes/deployment-and-submission.md). A
